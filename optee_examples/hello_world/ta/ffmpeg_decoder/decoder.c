...
static int decodePacket(AVPacket *packet, int flush) {
    ...
    do {
        ...
        if (len < 0) {
            ...
            DMSG("Error while decoding frame %d\n", picDecodeNumber);
            return len;
        }
        if (got_frame) {
            printf("Got frame %d\n", picDisplayNumber);
...
u32 broadwayInit() {
    ...
    if (!codec_ctx) {
        ...
        DMSG("Could not allocate video codec context\n");
        // exit(1);
    }
    ...
    if(!parser) {
        ...
        DMSG("Could not create H264 parser\n");
        // exit(1);
    }
    
    if (avcodec_open2(codec_ctx, codec, NULL) < 0) {
        // fprintf(stderr, "Could not open codec\n");
        DMSG("Could not open codec\n");
        // exit(1);
    }
    ...
    if (!frame) {
        ...
        DMSG("Could not allocate video frame\n");
        // exit(1);
    }
