                   ...
                   int log_offset, void *log_ctx)
{
    ...
    if (nscanf(str, "%d:%d%c", &q->num, &q->den, &c) != 2) {
        double d;
...
//     printf("Maybe this is acceptable...?\n");
...
//         	struct tm *tmptm = is_utc ? gmtime_r(&now, &tmbuf): (localtime_r(&now, &tmbuf));
//             struct tm dt2 = *tmptm;
